...
function UrgencyBar({ regionName }: { regionName: string }) {
  ...
  return (
    <>
      <div className="hazard-stripes hazard-anim h-2" aria-hidden="true" />
      <div className="bg-neutral-950 text-white text-xs sm:text-sm font-bold py-2.5 px-4 text-center flex items-center justify-center gap-2 tracking-wider uppercase">
        <span>
...
function LiveBadge() {
  ...
  return (
    <motion.div
      ...
      className="inline-flex items-center gap-2 bg-neutral-950 border-2 border-yellow-400 text-yellow-400 px-4 py-2 rounded-none text-sm font-bold mb-6 uppercase tracking-wider shadow-[4px_4px_0_0_#f5c518]"
    >
      ...
      {isLive ? `Live: ${driversAvailable} Drivers Available Now` : '24/7 Recovery Drivers On Call'}
    </motion.div>
  );
}
    ...
    <header className="sticky top-0 z-50 bg-white text-slate-950 border-b-2 border-slate-950">
      <div className="max-w-6xl mx-auto px-4 h-20 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-6">
          ...
          <a
            href={`tel:${PHONE_TEL}`}
            className="bg-yellow-400 hover:bg-yellow-300 text-neutral-950 px-7 py-3 rounded-none font-bold text-base uppercase tracking-wider transition-all active:scale-95 flex items-center gap-2 shadow-[4px_4px_0_0_#0a0a0a] hover:shadow-[2px_2px_0_0_#0a0a0a] hover:translate-x-0.5 hover:translate-y-0.5"
          >
...
function Hero({ regionName }: { regionName: string }) {
  return (
    <section className="relative bg-white text-slate-950 pt-8 lg:pt-20 pb-20 px-4 overflow-hidden border-b border-slate-200">
      ...
      <div className="relative z-10 max-w-6xl mx-auto grid lg:grid-cols-2 gap-12 items-center">
        <div>
          ...
          <motion.h1
            ...
            className="font-display text-[2.5rem] sm:text-6xl md:text-7xl tracking-tight mb-6 leading-[1.08]"
          >
            <span className="block text-xs md:text-sm text-red-600 mb-4 font-black tracking-[0.3em] uppercase font-sans">
              Stuck on the road?
            </span>
            BREAKDOWN
            <br />
            ...
            <motion.span
              ...
              className="inline-block origin-left mt-3 leading-[0.95] px-3 pt-1 pb-1.5 bg-yellow-400 text-neutral-950 shadow-[5px_5px_0_0_#141414]"
            >
          ...
          >
            <a
              href={`tel:${PHONE_TEL}`}
              className="sheen w-full bg-yellow-400 hover:bg-yellow-300 text-neutral-950 font-display py-5 px-6 rounded-none flex items-center justify-center gap-3 text-xl sm:text-2xl uppercase tracking-wider transition-all hover:-translate-y-0.5 hover:-translate-x-0.5 shadow-[8px_8px_0_0_#0a0a0a] hover:shadow-[10px_10px_0_0_#0a0a0a]"
            >
      ...
      <div className="max-w-6xl mx-auto flex flex-wrap justify-center gap-8 sm:gap-16">
        {TRUST_ITEMS.map((item) => {
          ...
          return (
            <div
              key={item.text}
              className="flex items-center gap-2 text-slate-600 font-semibold uppercase tracking-wider text-sm sm:text-base"
            >
...
function SeoContent({ regionName }: { regionName: string }) {
  return (
    <Reveal>
      <section className="py-16 px-4 max-w-3xl mx-auto prose prose-slate">
        <h2 className="font-display text-2xl mb-4 uppercase tracking-tight">
...
function Coverage() {
  return (
    <section className="py-20 px-4 bg-neutral-950 text-white border-t border-neutral-900">
      <div className="max-w-6xl mx-auto">
        <Reveal className="text-center mb-12">
          <div className="text-xs font-black tracking-[0.3em] uppercase text-red-500 mb-3">
            Coverage Map
          </div>
          <h2 className="font-display text-3xl md:text-5xl mb-4 uppercase tracking-tight">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 text-sm font-bold text-neutral-200">
          {REGIONS.filter((area) => area !== HOME_REGION).map((area, i) => (
            <Reveal key={area} delay={Math.min(i * 0.02, 0.5)} y={16}>
              <Link
                to={regionPath(area)}
                className="flex items-center gap-2 p-3 rounded-none bg-neutral-900 border border-neutral-800 hover:bg-yellow-400 hover:text-neutral-950 hover:border-yellow-400 hover:-translate-y-0.5 transition-all justify-center text-center"
              >
                {area}
              </Link>
            </Reveal>
...
function FinalCta({ regionName }: { regionName: string }) {
  return (
    <section className="py-24 px-4 bg-yellow-400 relative overflow-hidden">
      <div
        className="hazard-stripes hazard-anim absolute inset-x-0 top-0 h-3"
        aria-hidden="true"
      ></div>
      <div
        className="hazard-stripes hazard-anim absolute inset-x-0 bottom-0 h-3"
        aria-hidden="true"
      ></div>
      <Reveal className="max-w-4xl mx-auto text-center relative z-10">
        <h2 className="font-display text-4xl md:text-6xl text-neutral-950 mb-6 tracking-tight uppercase">
          Don't stay stranded.
        </h2>
        <p className="text-xl text-neutral-800 font-bold mb-12">
          Our {regionName} recovery team is ready to dispatch immediately.
        </p>
        <div className="flex flex-col sm:flex-row justify-center gap-4 sm:gap-6">
          <a
            href={`tel:${PHONE_TEL}`}
            className="sheen bg-neutral-950 hover:bg-black text-yellow-400 font-display py-4 px-8 rounded-none flex items-center justify-center gap-3 text-xl uppercase tracking-wider transition-all hover:-translate-y-0.5 hover:-translate-x-0.5 shadow-[6px_6px_0_0_#0a0a0a] hover:shadow-[8px_8px_0_0_#0a0a0a]"
          >
          ...
          <button
            ...
            className="bg-white text-neutral-950 hover:bg-neutral-950 hover:text-white border-2 border-neutral-950 font-display py-4 px-8 rounded-none flex items-center justify-center gap-3 text-xl uppercase tracking-wider transition-all hover:-translate-y-0.5"
          >
            Book Online Now
          </button>
        </div>
...
function Footer({ regionName }: { regionName: string }) {
  return (
    <footer className="bg-neutral-950 text-neutral-300 py-16 px-4 text-sm border-t-4 border-yellow-400">
      <div className="max-w-6xl mx-auto grid md:grid-cols-4 gap-12">
        <div className="col-span-2">
          <div className="flex items-center gap-3 mb-6">
            <Logo className="w-10 h-10 shrink-0" />
            <span className="font-display text-2xl tracking-tight text-white">
              24/7<span className="text-yellow-400">RECOVERY</span>
            </span>
          </div>
        ...
        <div>
          <h4 className="text-white font-display text-base mb-4 uppercase tracking-wider">
            Services
          </h4>
      ...
      <div className="max-w-6xl mx-auto mt-12 pt-8 border-t border-neutral-900 text-center font-medium flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-4">
        <p>© 2026 24/7 Breakdown Recovery {regionName}. All rights reserved.</p>
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 p-3 bg-white/95 backdrop-blur-md border-t border-slate-200 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] z-50 sm:hidden">
      <div className="flex gap-3">
        <a
          href={`tel:${PHONE_TEL}`}
          className="flex-[3] bg-yellow-400 text-neutral-950 font-bold py-3.5 rounded-sm flex items-center justify-center gap-2 active:scale-95 transition-transform shadow-md shadow-yellow-400/30"
        >
        ...
        <button
          type="button"
          onClick={scrollToBooking}
          className="flex-[2] bg-neutral-950 text-white font-bold py-3.5 rounded-sm flex items-center justify-center active:scale-95 transition-transform shadow-md"
        >
          Book Online
        </button>
      </div>
...
function RegionLanding({ regionName }: { regionName: string }) {
  ...
  return (
    <MetricsProvider>
      <div className="min-h-screen bg-slate-50 font-sans text-slate-950 pb-28 sm:pb-0 selection:bg-yellow-400 selection:text-neutral-950">
