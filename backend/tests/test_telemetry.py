...
def test_events_are_accepted_and_counted(client, admin_headers):
    assert send(client, event(), event("booking_started")).status_code == 204
    body = client.get("/api/admin/telemetry", headers=admin_headers).json()
    assert body["events"] == 2
    assert body["sessions"] == 1


def test_telemetry_needs_an_admin(client):
    assert client.get("/api/admin/telemetry").status_code == 401


def test_funnel_counts_visits_not_clicks(client, admin_headers):
    ...
    funnel = {s["name"]: s["sessions"] for s in
              client.get("/api/admin/telemetry", headers=admin_headers).json()["funnel"]}
    assert funnel["booking_started"] == 1


def test_funnel_percentages_are_relative_to_arrivals(client, admin_headers):
...
def test_quotes_are_summarised(client, admin_headers):
    send(
        client,
...
def test_regions_are_ranked(client, admin_headers):
    send(client, event(session="a", region="Bolton"), event(session="b", region="Bolton"))
    send(client, event(session="c", region="Sale"))
    regions = client.get("/api/admin/telemetry", headers=admin_headers).json()["topRegions"]
    assert regions[0]["label"] == "Bolton" and regions[0]["count"] == 2


def test_calls_are_counted_separately(client, admin_headers):
    send(client, event("call_clicked"), event("call_clicked", session="s2"))
    assert client.get("/api/admin/telemetry", headers=admin_headers).json()["callClicks"] == 2
...
def test_empty_window_does_not_error(client, admin_headers):
    body = client.get("/api/admin/telemetry?days=7", headers=admin_headers).json()
